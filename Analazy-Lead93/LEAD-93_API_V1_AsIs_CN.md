...
> 接口总数：18（LEAD-93 Contract 16、辅助查询 2）
...
| 标记 | 含义 |
|---|---|
| `QA 实测` | 2026-07-16、2026-07-21 已在 QA 发起真实请求并验证 HTTP、业务码、字段结构或业务结果 |
...
| 版本管理 | [V1-09](#v1-09-add-version)、[V1-10](#v1-10-update-version)、[V1-11](#v1-11-delete-version)、[V1-12](#v1-12-get-max-version)、[V1-16](#v1-16-get-next-version)、[V1-13](#v1-13-version-detail)、[V1-14](#v1-14-version-history) |
...
| `data.dataList[].updatedBy/updatedDate` | String/DateTime | 更新信息 |
...
```json
{
  ...
  "data": {
    ...
    "dataList": [{
      ...
      "updatedDate": "2026-07-16 09:00:00",
...
| `data.dataList[].channel/channelName` | String | Channel 编码和名称 |
| `data.dataList[].emailStatus` | Integer | Template 启停状态 |
| `data.dataList[].createdBy/createdDate` | String/DateTime | 创建信息 |
| `data.dataList[].updatedBy/updatedDate` | String/DateTime | 更新信息 |
| `data.dataList[].sendTimes` | Integer | 使用次数 |
| `data.dataList[].daeCountryCode` | String | 国家编码 |
...
```json
{
  ...
  "data": {
    ...
    "dataList": [{
      ...
      "updatedBy": "content.manager",
      "updatedDate": "2026-07-16 10:00:00",
      "sendTimes": 0,
      "daeCountryCode": "ZA"
    }]
  }
...
| 仅 Expired 且无 Draft | 目标下一版本、`isDraft="1"` | 保留 Expired V(N)，Insert V(N+1) Draft |
...
正常页面在已有 Draft 或 Schedule 时禁止再次创建 Draft，因此标准前端路径通常只有一个 Draft。现有后端和数据库没有唯一 Draft 保护：直接调用、并发请求或异常数据下可能出现多个 Draft；在 V2/V3 均为 Draft且请求 V4 不存在时，代码可能继续 Insert V5。该行为是现状风险，不是受支持的接口契约。
...
| Response DTO | Controller 声明为 `TemplateEmailVersionVO`；QA 运行时 `data=null` |
...
QA 实测表明接口成功后 `data=null`。客户端必须通过 `getMaxVersion`、`version/detail` 或 Version History 确认最终版本状态。
...
- 本接口表达现有“增加同一 Template 版本并切换 Active”语义；To-Be Copy and Create 使用独立 v2 API，不从本接口推导。
...
QA 实测返回 `TemplateEmailVersionVO` 外形，但其中 `id/version/versionStatus/emailCode` 均为 `null`。客户端不得依赖响应对象确认更新结果。
...
### V1-16 Get Next Version

**接口与场景**

| 项目 | 内容 |
|---|---|
| Endpoint | `GET /iic-dae-msg/web/msg/template/email/v1/version/getNextVersion?emailCode={emailCode}` |
| 当前场景 | 获取下一个可用的数字版本号 |
| Request DTO | Query Parameter `emailCode` |
| Response DTO | `String` |
| 证据 | `QA 实测` + `代码确认` |
...
- 返回值遵循 `Vn` 格式，不使用 `1.0/2.0`。
- 该接口只计算下一个版本号，不创建 Version 记录。
...
```json
{
  ...
  "data": {
    ...
    "dataList": [
      ...
      {"emailCode":"815645091883520000","version":"V1","versionStatus":2,"updatedBy":"content.manager","updatedDate":"2026-07-16 09:00:00"}
    ]
  }
}
```
...
| `data.dataList[].isDeceased` | Boolean | 是否已故；QA 实测序列化为 Boolean |
| `data.dataList[].allEmailDetail` | Object[] | 邮件详情集合；QA 实测序列化为 Array |
...
| 指标 | 结果 |
|---|---:|
| Contract Endpoint | 16/16 已调用（含独立 `getNextVersion` 探针） |
...
2026-07-21 追加只读 Contract Probe：16 个请求、86 个断言全部通过。其中分页 A/B 测试确认 `templateList/queryList/recipientList` 的请求字段均为 `pageNum`；传 `pageNo=2` 时响应仍为第一页。`templateList` 另确认 `templateStatus/channelList/emailStatusList/sortField/isAsc` 会参与查询。当前 `queryList` 在 QA 的基线结果为 0 条，因此其 `channelList/isCampaign/querySort` 精确过滤效果仍未由本轮数据证明。
...
- [v1 Contract 差异探针 Collection](postman/LEAD-93-v1-contract-probes.postman_collection.json)
- [2026-07-21 v1 完整回归脱敏 HTML](postman/reports/v1-full-run-2026-07-21_132810.html)
- [2026-07-21 v1 差异探针脱敏 HTML](postman/reports/v1-contract-probes-2026-07-21_132746.html)
- [Newman 测试与脱敏报告说明](postman/README.md#7-newman--htmlextra-qa-证据)
