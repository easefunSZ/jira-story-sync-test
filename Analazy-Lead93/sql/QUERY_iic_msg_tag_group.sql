...
WHERE c.status = 0 AND c.email_status = 1 AND g.is_mandatory = 1
  AND NOT EXISTS (
    SELECT 1
    FROM iic_msg_template_tag_rel r
    JOIN iic_msg_tag_value tv ON tv.tag_code = r.tag_code
    WHERE r.email_code = c.email_code AND tv.group_code = g.group_code
  );
