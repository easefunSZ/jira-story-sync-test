...
SELECT c.id, c.category_name, c.parent_id,
       CASE WHEN c.parent_id = 0 THEN 1 ELSE 2 END AS category_level,
       c.sort_order
FROM iic_msg_email_category c
WHERE c.is_deleted = 0
ORDER BY CASE WHEN c.parent_id = 0 THEN 1 ELSE 2 END, c.parent_id, c.sort_order, c.id;

-- Batch Subcategory Create: lock and validate an active level-1 parent.
SELECT c.id, c.parent_id, c.is_deleted
FROM iic_msg_email_category c
WHERE c.id = :parent_id AND c.parent_id = 0 AND c.is_deleted = 0
FOR UPDATE;
...
FROM iic_msg_email_category c
LEFT JOIN iic_msg_email_category p ON p.id = c.parent_id AND p.parent_id = 0 AND p.is_deleted = 0
WHERE c.is_deleted = 0 AND c.parent_id <> 0 AND p.id IS NULL;
...
SELECT src.id, CASE WHEN src.parent_id = 0 THEN 1 ELSE 2 END AS category_level,
