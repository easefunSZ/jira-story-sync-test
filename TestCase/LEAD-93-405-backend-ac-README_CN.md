...
### Mac 通过远程 Bash 执行数据库断言

若 Mac 可访问应用 API、但不能直连测试 MySQL，使用独立入口。Newman 与报告在 Mac 执行；每个数据库检查点通过远程 Bash WebSocket 下发，远程 Python 从运行服务使用的 `application-*.properties` 读取数据源并执行只读断言。数据库密码不会传回 Mac，也不写入报告。

```bash
cd /Users/qthitsz/DAE-workspace/om/Lead-93/TestCase/postman
cp remote-db-test.env.example remote-db-test.env
# 只需填写 REMOTE_DB_PROPERTIES_PATH
./run-lead93-lead405-ac-with-remote-db.sh local-dev-LEAD-93-405-backend-ac.postman_environment.json
```

`remote-db-test.env` 是普通配置文件且不纳入 Git；无需执行 `export`。`REMOTE_DB_PROPERTIES_PATH` 必须是**远程正在运行服务实际使用**的数据源配置文件；该值是路径，不是数据库密码。第二个命令参数可替换默认配置文件。Mac 环境文件的 `baseUrl` 必须指向 Mac 可访问的 Dev 服务地址；除非已经做端口转发，不要填写远程机器内部的 `http://localhost:8086`。远程端需要已安装 `python` 与 `pymysql`，并可连接测试 MySQL。
...
每条 Jira AC 不再混在 Story 摘要中。请以逐条表格 [LEAD-93-405_AC_Traceability_CN.md](LEAD-93-405_AC_Traceability_CN.md) 为准；一键报告会使用同一份追溯数据，在每次执行后对每条 AC 输出 `PASS`、`PASS（后端范围）`、`FAIL`、`NOT RUN`、`NOT COVERED` 或 `N/A UI`。
...
| AC2 | 字段约束：模板名称、分类单选、子分类多选且归属正确 | API-17, API-20, API-28, API-29, API-58 | DB-04, DB-06 | FULL | - |
| AC3 | Save Draft 允许 Metadata 未完成，状态为 Draft | API-18, API-19 | DB-01 | FULL | - |
| AC4 | 草稿未选必填 Tag Group 时保持空选择 | API-18 | DB-01 | FULL | - |
...
| AC8 | 模板名称必填且不超过 120 字符 | API-17, API-58 | - | FULL | - |
...
| AC14 | 所有发布入口复用一致的发布校验 | API-20, API-45 | - | BACKEND_SCOPE | 首次发布和 Version 发布已验证；Preview 页面入口属于前端专属。 |
...
| AC2 | 仅可选当前 Category 下 Subcategory，切换 Category 清空选择 | API-29 | DB-06 | BACKEND_SCOPE | 后端已拒绝跨分类 Subcategory；页面清空行为属于前端。 |
| AC3 | Draft 可不选 Category/Subcategory，状态保持 Draft | API-18, API-19 | DB-01 | FULL | - |
| AC4 | Category 与同父 Subcategory 可排序，不能跨父移动 | API-15 | - | BACKEND_SCOPE | 完整同级排序写入已验证；拖拽禁用与跨父交互属于前端。 |
| AC5 | Category/Subcategory 持久化，并可在列表、搜索和导航定位 | API-22, API-23, API-26, API-31 | DB-04, DB-06 | FULL | - |
| AC6 | Draft 修改分类后立即保存并更新位置 | API-59, API-60 | DB-20 | FULL | - |
| AC7 | Published 修改分类为 Metadata 变更且仍为 Published | API-24, API-25, API-26 | DB-04 | FULL | - |
| AC8 | Category/Subcategory 名称必填且最长 100 字符 | API-5, API-10, API-11, API-56, API-57 | - | FULL | - |
| AC9 | Category/Subcategory 有效名称全局重名被拒绝且不落部分数据 | API-9, API-11, API-55 | DB-07 | FULL | - |
...
| AC4 | Publish 严格校验、Draft 转 Published、Adviser 可见、Copy 复用规则 | API-20, API-21, API-23, API-32, API-45 | DB-03, DB-10 | BACKEND_SCOPE | 后端状态和可见性查询已验证；页面消息和 Adviser UI 不在本套范围。 |
| AC5 | 仅 Content Manager 可创建 Template | API-65 | - | CONDITIONAL | 需配置 adviserAuthorization 并将 runPermissionTests=true。 |
...
| AC3 | 有引用时阻止直接删除并提供迁移选择 | API-50, API-51 | DB-12, DB-13 | FULL | - |
| AC4 | 无引用 Category 级联软删除子节点并从树隐藏 | API-16, API-51, API-53 | DB-12, DB-21 | FULL | - |
| AC5 | 无引用 Subcategory 单独软删除且不影响父 Category | API-48, API-49, API-61, API-62, API-63 | DB-22 | FULL | - |

### LEAD-276

| AC | 具体验收内容 | Postman 用例 | 数据库断言 | 覆盖度 | 备注 |
| --- | --- | --- | --- | --- | --- |
| AC1 | 编辑页回显已选 Category/Subcategory | API-22, API-26, API-28 | DB-04 | BACKEND_SCOPE | Detail 回显已验证；选中态渲染属于前端。 |
| AC2 | 改 Category 清空旧 Subcategory 并刷新候选项 | API-29 | DB-06 | BACKEND_SCOPE | 后端拒绝跨分类关系；页面自动清空与刷新属于前端。 |
...
| AC7 | Draft 修改分类后立即保存 | API-59, API-60 | DB-20 | FULL | - |
| AC8 | 重新分类后搜索、筛选和位置同步更新 | API-23, API-31, API-47, API-52 | DB-13 | FULL | - |
| AC9 | Publish 必填分类，Draft 可为空 | API-18, API-20 | DB-01, DB-03 | FULL | - |
| AC10 | 仅 Content Manager 可修改分类 | API-66 | - | CONDITIONAL | 需配置 adviserAuthorization 并将 runPermissionTests=true。 |

### LEAD-278

| AC | 具体验收内容 | Postman 用例 | 数据库断言 | 覆盖度 | 备注 |
| --- | --- | --- | --- | --- | --- |
| AC1 | 从 Published Copy 创建独立 Draft，原模板保持可用 | API-32, API-34 | DB-08, DB-09 | BACKEND_SCOPE | 独立 Copy 与原模板状态已验证；WYSIWYG 预填渲染属于前端。 |
| AC2 | WYSIWYG 格式化能力 | - | - | UI | - |
| AC3 | 手动 Save Draft 保存 B，原模板不受影响 | API-33, API-34 | DB-08, DB-09 | BACKEND_SCOPE | 保存和原模板不变已验证；页面成功消息与 Version History 文案属于前端。 |
...
| AC2 | 创建 Category/Subcategory 名称，最长 100 字符 | API-6, API-12, API-13, API-56, API-57 | - | FULL | - |
| AC3 | 空/重复名称失败，单次最多五个 Subcategory，无部分落库 | API-5, API-9, API-10, API-11, API-55 | DB-07 | FULL | - |
| AC4 | 创建后立即在下拉和导航可用 | API-2, API-6, API-12, API-13 | - | BACKEND_SCOPE | Tree API 可见性已验证；页面下拉渲染属于前端。 |
| AC5 | 新分类默认有效并可立即被 Template 使用 | API-6, API-21 | DB-04 | FULL | - |
| AC6 | 创建失败无部分数据 | API-10, API-11 | DB-07 | FULL | - |
| AC7 | 仅 Content Manager 可创建/编辑 Category | API-64 | - | CONDITIONAL | 需配置 Adviser 登录态。 |
| AC8 | 创建人和创建时间被保存 | - | DB-19 | FULL | - |
...
| AC | 具体验收内容 | Postman 用例 | 数据库断言 | 覆盖度 | 备注 |
| --- | --- | --- | --- | --- | --- |
| 1 | 返回 Tag Group 与 Required 标识 | API-1 | DB-05 | BACKEND_SCOPE | Taxonomy 返回已验证；页面展示属于前端。 |
| 2 | Tag 仅可从预置多选 taxonomy 选择，不接受自由文本 | API-1, API-30 | DB-05 | FULL | - |
| 3 | Publish 缺少任一必填 Tag Group 时阻止发布 | API-1, API-20 | DB-03, DB-05 | FULL | - |
| 4 | Save Draft 不强制 Tag，未选值保持空关系 | API-18 | DB-01 | FULL | - |
| 5 | 每个 Tag Group 支持多选 | API-1, API-24, API-26 | DB-05 | FULL | - |
| 6 | 可回显并增删已分配 Tag | API-24, API-26 | DB-05 | FULL | - |
| 7 | Tag 是 Metadata；修改 Published Tag 不改变状态且不受分类限制 | API-24, API-25 | DB-04, DB-05 | FULL | - |
| 8 | 仅 Content Manager 可编辑 Tag，Adviser 只读 | API-66 | - | CONDITIONAL | 需配置 adviserAuthorization 并将 runPermissionTests=true。 |
...
| `postman/remote-db-test.env.example` | Mac 远程数据库断言配置样例；实际 `remote-db-test.env` 不纳入版本控制。 |
| `run-lead93-lead405-ac-newman.sh` | 生成集合、运行 Newman、产出调试报告。 |
| `run-lead93-lead405-ac-with-db.sh` | 内网一键执行 API 分阶段回归、只读 MySQL 断言和合并报告。 |
| `run-lead93-lead405-ac-with-remote-db.sh` | Mac 执行 API 回归、通过远程 Bash + Python 执行只读数据库断言并生成合并报告。 |
| `scripts/run-db-ac-checks-via-remote-terminal.mjs` | 远程数据库断言适配器；只传递只读 SQL 和运行时 ID，不传递数据库凭证。 |
