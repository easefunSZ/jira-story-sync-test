...
function stepStatus(step) {
  ...
  const result = dbResults.get(String(step.evidence.id));
...
const storySections = (scenarioManifest.stories ?? []).map(storyDefinition => {
  ...
  return `<section class="story"><h2>${escapeHtml(story)}</h2><p class="story-note">以下场景和步骤来自显式测试场景基线；API 和数据库执行证据可以被多个 AC 复用，但步骤顺序不会动态改变。</p>${scenarioBlocks || '<p class="note">该 Story 当前没有可执行的后端场景。</p>'}<h3>AC 与固定场景步骤映射</h3><div class="scroll"><table><thead><tr><th>AC</th><th>验收要求</th><th>关联场景</th><th>对应步骤</th><th>AC 结果</th><th>结论依据</th></tr></thead><tbody>${acRows}</tbody></table></div></section>`;
}).join("\n");
...
const apiRows = entries.filter(entry => entry.type === "api").map(entry => {
  ...
  return `<tr><td>${escapeHtml(entry.stage)}</td><td class="${css}">${escapeHtml(entry.status)}</td><td>${requests}</td><td>${assertions}</td><td>${issueCount}</td><td class="reason">${reasonHtml(blocking)}</td><td><a href="${escapeHtml(relativeLink(debug))}">Private Debug</a> | <a href="${escapeHtml(relativeLink(summary))}">Sanitized Summary</a></td></tr>`;
}).join("\n");
