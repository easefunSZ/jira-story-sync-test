...
RUN_REPORT_DIR="$ROOT_REPORT_DIR/lead93-405-406-407-308-remote-$STAMP"
RUN_PRIVATE_DIR="$ROOT_PRIVATE_DIR/lead93-405-406-407-308-remote-$STAMP"
BASE_RUNNER="$POSTMAN_DIR/run-lead93-lead405-ac-with-remote-db.sh"
...
if [[ ! -f "$REMOTE_DB_CONFIG_FILE" ]]; then
  echo "Remote DB config file not found: $REMOTE_DB_CONFIG_FILE" >&2
  exit 2
fi
...
echo "== Phase 1: LEAD-93 / LEAD-405 API and remote database AC verification =="
set +e
REPORT_DIR="$RUN_REPORT_DIR" NEWMAN_PRIVATE_DIR="$RUN_PRIVATE_DIR" "$BASE_RUNNER" "$ENV_FILE" "$REMOTE_DB_CONFIG_FILE"
CORE_STATUS=$?
set -e

CORE_REPORT="$(find "$RUN_REPORT_DIR" -maxdepth 1 -name 'lead93-405-backend-ac-with-remote-db-*.html' -print -quit)"
...
echo "All executable API and remote database checks passed. OPEN/UI ACs remain visible in the five-feature overview."
