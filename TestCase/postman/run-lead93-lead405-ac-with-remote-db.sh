...
REMOTE_DB_CONFIG_FILE="${2:-$POSTMAN_DIR/remote-db-test.env}"
...
FINAL_REPORT="$REPORT_DIR/lead93-405-backend-ac-with-remote-db-${STAMP}.html"
HAS_CONTRACT_ISSUES=0

load_remote_db_config() {
  local config_file="$1"
  [[ -f "$config_file" ]] || return 0
  local original_line line key value
  while IFS= read -r original_line || [[ -n "$original_line" ]]; do
    line="${original_line#${original_line%%[![:space:]]*}}"
    ...
    if [[ "$value" == \'*\' && "$value" == *\' ]]; then value="${value:1:${#value}-2}"; fi
    case "$key" in
      REMOTE_BASH_WS|REMOTE_DB_PROPERTIES_PATH|REMOTE_DB_TIMEOUT_MS)
        if [[ -z "${!key:-}" ]]; then printf -v "$key" '%s' "$value"; fi
        ;;
    esac
  done < "$config_file"
  export REMOTE_BASH_WS REMOTE_DB_PROPERTIES_PATH REMOTE_DB_TIMEOUT_MS
}
...
load_remote_db_config "$REMOTE_DB_CONFIG_FILE"
if [[ -z "${REMOTE_DB_PROPERTIES_PATH:-}" ]]; then
  echo "Configure REMOTE_DB_PROPERTIES_PATH in $REMOTE_DB_CONFIG_FILE." >&2
  exit 2
fi
...
run_api_phase() {
  ...
  set +e
  "${NEWMAN[@]}" run "$COLLECTION" -e "$RUNTIME_ENV" --export-environment "$RUNTIME_ENV" --folder "$folder" --reporters cli,json --reporter-json-export "$raw" --timeout-request 30000
  local status=$?
  set -e
  if [[ -f "$raw" ]]; then
...
run_db_checkpoint() {
  ...
  echo "== Remote database checkpoint: $checkpoint =="
  set +e
  node "$POSTMAN_DIR/scripts/run-db-ac-checks-via-remote-terminal.mjs" --environment "$RUNTIME_ENV" --template "$ASSERTION_SQL" --checkpoint "$checkpoint" --output "$output"
  local status=$?
  set -e
  local result="PASS"
...
echo "All API and remote database acceptance checks passed."
