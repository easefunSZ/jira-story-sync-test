...
Newman 主流程使用 fail-fast：任一 HTTP、业务码、响应结构或业务断言失败时立即停止，后续依赖请求不再执行。默认情况下**测试失败时保留现场（不自动执行清理）**，方便开发与 QA 快速查验数据库与中间数据定位问题。若需强制清理现场，可在运行脚本时指定环境变量 `CLEAN_ON_FAILURE=true`。
...
```text
skipRequestNames=Delete Unreferenced Category,Check Referenced Source Category Impact
skipSteps=10,41
```
...
```bash
SKIP_STEPS=10,41 Lead-93/postman/run-v2-qa-newman.sh /absolute/path/environment.json
```

或按名称指定：

```bash
SKIP_REQUEST_NAMES="Delete Unreferenced Category,Check Referenced Source Category Impact" \
